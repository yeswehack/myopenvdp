...
async function captchaProvider() {
  ...
  return await fetch('https://my-open-vdp-app.localhost/api/captcha?w=200&h=56').then((response) => response.json());
}
...
function logSubmit(
  ...
  fetch('https://my-open-vdp-app.localhost/api/upload/captcha', config)
    .then(async (response) => {
      if (!response.ok) {
