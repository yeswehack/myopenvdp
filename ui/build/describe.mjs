...
function mdGetEventParams(event) {
  const params =
    event.params === void 0 || event.params.length === 0 ? '' : Object.keys(event.params).map(mdCode).join(', ');
...
function mdDescribeProp(writer, prop, propName, indentLevel) {
  ...
  if (propName !== void 0) {
    writer.writeLine(
      `${indent}- ${mdBold(mdCode(propName))} ${type ? `(${type})` : ''}${
        type !== 'Function' && prop.required ? ' [Required]' : ''
  ...
  if (type === 'Function') {
    writer.writeLine(
      `${indent}- ${mdBold('Function form')}: ${mdCode(getMethodParams(prop, true) + getMethodReturnValue(prop))}`
    );
  }
  if (prop.values) {
